"""
以下代码来自：
- https://github.com/fredzzhang/pocket/blob/master/pocket/ops/transforms.py
- https://github.com/facebookresearch/detr/blob/main/datasets/transforms.py
"""
import random
import PIL
import torch
...
from hoidet.utils.box_ops import box_xyxy_to_cxcywh
from hoidet.utils.misc import interpolate


__all__ = [
    'to_list_of_tensor', 'to_tuple_of_tensor', 'to_dict_of_tensor'
]


def to_list_of_tensor(x, dtype=None, device=None):
    return [torch.as_tensor(item, dtype=dtype, device=device) for item in x]


def to_tuple_of_tensor(x, dtype=None, device=None):
    return tuple(torch.as_tensor(item, dtype=dtype, device=device) for item in x)


def to_dict_of_tensor(x, dtype=None, device=None):
