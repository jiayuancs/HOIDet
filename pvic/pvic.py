...
class HumanObjectMatcher(nn.Module):
    def __init__(self, repr_size, num_verbs, obj_to_verb, dropout=.1, human_idx=0):
        ...
        self.encoder = TransformerEncoder(dim=256, layer_num=2, dropout=dropout, pos_dim=512)
...
class FeatureHead(nn.Module):
    def __init__(self, dim, dim_backbone, return_layer, num_layers):
        ...
        self.layers = nn.Sequential(
            ...
            # TransformerEncoder(dim=dim, layer_num=2)
        )
    ...
    def forward(self,
        ...
        for i, (ho_q, mem) in enumerate(zip(ho_queries, memory)):
            query_embeds.append(self.decoder(
                query=ho_q.unsqueeze(1),  # (n, 1, q_dim)
                key_value=mem.unsqueeze(1),  # (hw, 1, kv_dim)
                ca_key_padding_mask=~kv_p_m[i].t(),  # (hw, 1)
                sa_pos=positional_embeds[i]["box"] if "box" in positional_embeds[i].keys() else None,  # centre: (n, 1, 2*kv_dim), box: (n, 1, 4*kv_dim)
                ca_query_pos=positional_embeds[i]["centre"] if "centre" in positional_embeds[i].keys() else None,
                ca_key_pos=k_pos[i]  # (hw, 1, kv_dim)
            ).squeeze(dim=2))
...
def build_detector(args, obj_to_verb):
    ...
    detr_object = DETR(args)
    detr_object.freeze_params()
    detr = detr_object.detector
    ...
    decoder_layer = TransformerDecoderLayer(
        query_dim=args.repr_dim, cross_kv_dim=args.hidden_dim,
        ffn_interm_dim=args.repr_dim * 4,
        head_num=args.nheads, dropout=args.dropout,
        sa_pos_dim=args.hidden_dim * 4,
        ca_query_pos_dim=args.hidden_dim * 2,
        ca_key_pos_dim=args.hidden_dim
    )
    triplet_decoder = TransformerDecoder(
        decoder_layer=decoder_layer,
        layer_num=args.triplet_dec_layers
    )
    ...
    model = PViC(
        (detr, args.detector), detr_object.postprocessors,
        ...
        min_instances=args.min_instance_num,
        max_instances=args.max_instance_num,
